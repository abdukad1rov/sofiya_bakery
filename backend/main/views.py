...
def branch_orders_view(request, branch_id=None):
    ...
    grouped_orders = {
        'Waiting': orders.filter(isAccepted=False, isCanceled=False),
        'In Process': orders.filter(isProcess=True),
        'Delivered': orders.filter(isDelivered=True),
        'Completed': orders.filter(isCompleted=True),
        'Canceled': orders.filter(isCanceled=True),
    }
